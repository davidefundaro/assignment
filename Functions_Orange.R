...
add_origin.lien_guarantees <- function(data) {
  result <- data %>%
    mutate(origin.lien = case_when(
      str_detect(origin.lien, "volon")  ~ "voluntary",
      str_detect(origin.lien, "giudi|surroghe")  ~ "judicial",
      TRUE ~ NA_character_
    ))
} 


add_type_guarantees <- function(data) {
  result <- data %>%
    mutate(type = case_when(
      str_detect(type, "fideju|fideiu|fidi|consortile")  ~ "surety",
      str_detect(type, "lien")  ~ "lien",
      TRUE ~ NA_character_
    ))
} 
...
add_status_guarantees <- function(data, date_column, threshold_years) {
  data %>%
    mutate(
      years_difference = as.numeric(difftime(Sys.Date(), !!sym(date_column), units = "days")) / 365.25,
      status = case_when(
        years_difference >= threshold_years ~ "expired",
        TRUE ~ "valid"
      )
    ) %>%
    select(-years_difference)  # Optionally, remove the intermediate years_difference column
}
#example:
#guarantees <- add_status_guarantees(guarantees, "date.registration", 20)


#agreement_summary status
add_status_agreement_summary <- function(data, six.m.date) {
  data <- data %>%
    mutate(cutoff_date = as.Date(six.m.date, format= "%Y-%m-%d"), 
           status = case_when(
             residual > 0 & paid == 0 & date.last.payment > cutoff_date ~ "proposal",
             residual == 0  ~ "closed",
             residual > 0 & paid > 0 & date.last.payment > cutoff_date ~ "active",
             residual > 0 & date.last.payment <= cutoff_date ~ "failed",
             TRUE ~ NA_character_
           )) %>% select(-cutoff_date)
  return(data)
}
#example:
#agreement_summary <- add_status_agreement_summary(agreement_summary, "2021-02-28")


#set agreement_summary column classes
agreement_summary_column_class <- function(data) {
  #numeric
  columns_numeric <- c("gbv.agreement", "amount.agreement", "paid", "residual")
  data[columns_numeric] <- lapply(data[columns_numeric], as.numeric)
  #factor
  columns_factor <- c("status")
  data[columns_factor] <- lapply(data[columns_factor], as.factor)
  #character
  columns_character <- c("id.agreement", "id.counterparty", "id.bor", "id.group")
  data[columns_character] <- lapply(data[columns_character], as.character)
  #date
  columns_date <- c("date.agreement", "date.start", "date.end", "date.last.payment")
  data[columns_date] <- lapply(data[columns_date], as.Date)
  #integer
  columns_integer <- c("year.agreement", "n.instalment", "length")
  data[columns_integer] <- lapply(data[columns_integer], as.integer)
  return(data)
}
#example:
#agreement_summary_final <- agreement_summary_column_class(agreement_summary_final)




#set agreement_proj column classes
agreement_proj_column_class <- function(data) {
  #numeric
  columns_numeric <- c("amount.due", "amount.paid")
  data[columns_numeric] <- lapply(data[columns_numeric], as.numeric)
  #character
  columns_character <- c("id.agreement")
  data[columns_character] <- lapply(data[columns_character], as.character)
  #date
  columns_date <- c("date.due", "date.paid")
  data[columns_date] <- lapply(data[columns_date], as.Date)
  return(data)
}
#example:
#agreement_proj_final <- agreement_proj_column_class(agreement_proj_final)




#check the borrowers which are at risk for the payment
add_risk_payment_agreement_summary <- function(data, one.m.date) {
  data <- data %>%
    mutate(cutoff_date = as.Date(one.m.date, format= "%Y-%m-%d"), 
           track = case_when(
             residual > 0 & status == "active" & date.last.payment < cutoff_date ~ "at risk",
             residual > 0 & status == "active" & date.last.payment >= cutoff_date ~ "on track",
             TRUE ~ NA_character_
           )) %>% select(-cutoff_date)
  return(data)
}
#agreement_summary_with_risk_payment <- add_risk_payment_agreement_summary(agreement_summary_with_risk_payment, "2021-07-31")
